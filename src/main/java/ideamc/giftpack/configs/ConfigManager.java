...
public final class ConfigManager<C> {
    ...
    public void reloadConfig() {
        try {
            ...
            GiftPackMain.getInstance().getLogger().severe("The yaml syntax in your configuration is invalid. "
                    + "Check your YAML syntax with a tool such as https://yaml-online-parser.appspot.com/");
            ex.printStackTrace();
        } catch (InvalidConfigException ex) {
            configData = configHelper.getFactory().loadDefaults();
            GiftPackMain.getInstance().getLogger().severe("One of the values in your configuration is not valid. "
